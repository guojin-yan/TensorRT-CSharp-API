...
public sealed class GpuAllocatorLocalPackageConsumerTests
{
    ...
    public void ConsumerTemplateUsesOnlyManagedAndBridgePackages()
    {
        ...
        string program = ReadSource("samples", "GpuAllocator.PackageConsumer", "Program.cs");
    ...
    {
        string script = ReadSource("eng", "Test-GpuAllocatorLocalPackageConsumer.ps1");

        foreach (string required in new[]
        {
            "samples\\GpuAllocator.PackageConsumer\\Program.cs",
            "GpuAllocator.PackageConsumer.csproj.template",
            "<clear />",
