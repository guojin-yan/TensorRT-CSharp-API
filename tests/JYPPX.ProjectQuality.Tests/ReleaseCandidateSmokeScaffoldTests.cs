...
public sealed class ReleaseCandidateSmokeScaffoldTests
{
    ...
    public void TrtexecOptionCoverageStaysBackedByMatrixParserAndTests()
    {
        ...
        Assert.Contains(entries, entry => entry.GetProperty("option").GetString() == "--loadInputs" && entry.GetProperty("implementationClass").GetString() == "applied-engine-order-input-binding");
        Assert.Contains(entries, entry => entry.GetProperty("option").GetString() == "--dumpOutput" && entry.GetProperty("implementationClass").GetString() == "applied-bounded-output-capture");
        Assert.Contains(entries, entry => entry.GetProperty("option").GetString() == "--dumpRawBindingsToFile" && entry.GetProperty("implementationClass").GetString() == "applied-bounded-output-capture");
        Assert.Contains(entries, entry => entry.GetProperty("option").GetString()!.Contains("--referenceOutputs", StringComparison.Ordinal) && entry.GetProperty("implementationClass").GetString() == "applied-name-shape-count-value-comparison");
    }
