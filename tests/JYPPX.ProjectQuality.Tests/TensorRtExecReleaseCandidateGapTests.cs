...
public sealed class TensorRtExecReleaseCandidateGapTests
{
    ...
    public void TensorRtExecReleaseCandidateGapListCapturesPublishableWorkItemsAndProofBoundaries()
    {
        ...
        Assert.Contains(items, static item => item.GetProperty("id").GetString() == "timing-cache" && item.GetProperty("currentStatus").GetString() == "parse-report-only-gui-cli" && item.GetProperty("winFormsSupported").GetBoolean());
