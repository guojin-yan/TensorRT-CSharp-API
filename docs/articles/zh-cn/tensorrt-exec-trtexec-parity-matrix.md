...
| 18 | input/output binding metadata | `--loadInputs`、`--dumpOutput`、`--dumpRawBindingsToFile` | implemented-pointer-free-binding-report | YoloVision `bindingMetadata`、`TensorRtEngineBindingReport`、artifact exporters | copied binding metadata 不是输出正确性或 package proof | 对 YoloVision real asset manifest 记录 input/output tensor metadata、semantic role 和 profile shape | 仅真实模型 smoke 通过后可作为 sample evidence，不是 package proof |
