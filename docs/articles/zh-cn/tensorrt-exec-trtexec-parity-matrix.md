...
| 10c | I/O 与 layer precision policy | `--inputIOFormats`、`--outputIOFormats`、`--precisionConstraints`、`--layerPrecisions`、`--layerOutputTypes` | implemented-build-readback-with-version-guards | `TrtexecLikeBuildPolicy`、tensor type/format、constraint flags、layer precision/output type、`TrtexecBuildPolicy` log | TRT8/10 set/readback 只证明 parsed network 上的 builder policy；TRT11 只能对 type 已匹配的 I/O 设置 format，移除的 setters 保持 parse-only | 用真实模型补 caller layout、expected output 和 owner hash，不能靠 identity smoke 晋级 | 否 |
| 10d | Engine packaging、refit 与 weight streaming | `--versionCompatible`、`--excludeLeanRuntime`、`--stripWeights`、`--refit`、`--allowWeightStreaming`、`--weightStreamingBudget` | implemented-build-runtime-readback-with-version-guards | packaging/refit builder flags、runtime host-code policy、engine refittable 与 V2 budget/scratch readback、YOLOX-S weighted enqueue | TRT8 strip/streaming 与 version-compatible+refit 冲突保持 parse-only；TRT11 本机 dependency-probe-only；本地 readback 不是跨版本 lean runtime、stripped-plan refit 或模型精度证明 | 另行完成外部 lean runtime 与 stripped-plan refit 生命周期；保留 compact evidence 的严格验证 | 否 |
...
| 12a | bounded benchmark scheduler | `--iterations`、`--warmUp`、`--duration`、`--streams`、`--infStreams`、`--avgRuns`、`--percentile`、`--threads`、`--useSpinWait`、`--useCudaGraph`、`--noDataTransfers` | implemented-bounded-runtime | `RunBoundedBenchmark`、`OnnxEngineBenchmarkSummary`、times artifact | 每个 effective stream 使用独立 context/bindings/CUDA stream/event；可选独立 driver thread、event polling、graph capture/launch 或 fallback、零 H2D/D2H；次数和时长均为最低条件 | no-transfer 不声明 output match；继续由模型 runner 提供 expected output，不把 scheduler evidence 当成模型正确性 | 否 |
| 13 | wait / idle benchmark controls | `--sleepTime`、`--idleTime` | partial | `TrtexecLikeRuntimeOptions`、`RunBoundedBenchmark`、`OptionImplementationStatus` | `--idleTime` 已在连续测量轮次间应用；`--sleepTime` 的 launch-to-compute gap 仍未实现 | 为 `--sleepTime` 设计忠实的 GPU scheduling 机制，不能用普通 CPU sleep 冒充 | 否 |
| 14 | layer dump | `--dumpLayerInfo`、`--exportLayerInfo` | implemented-inspector-readback | `ExportLayerInfoPath`、copied engine inspector layer text、report 诊断 | external ONNX 未必能提供完整 layer semantic proof；文本不是 runtime proof | 补兼容主机/owner 的真实 engine readback 证据 | 否 |
...
| 18 | input/output binding metadata | `--loadInputs`、`--dumpOutput`、`--dumpRawBindingsToFile` | implemented-pointer-free-binding-and-multi-output-artifacts | `OnnxEngineRuntimeOutputArtifact`、raw offset/hash manifest、YoloVision `bindingMetadata` | bounded capture 与 copied binding metadata 都不是 reference validation 或 package proof | 将多输出 JSON/raw manifest 与 YoloVision task contract、expected output 和 owner hashes 配对 | 仅真实模型 smoke 与 expected output 验证通过后可作为 sample evidence，不是 package proof |
