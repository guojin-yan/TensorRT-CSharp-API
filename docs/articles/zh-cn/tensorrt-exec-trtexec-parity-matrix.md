...
| 12a | bounded benchmark scheduler | `--iterations`、`--warmUp`、`--duration`、`--streams`、`--infStreams`、`--avgRuns`、`--percentile` | implemented-bounded-runtime | `RunBoundedBenchmark`、`OnnxEngineBenchmarkSummary`、times artifact | 每个 effective stream 使用独立 context/bindings/CUDA stream/event；次数和时长均为最低条件，平均窗口与 percentile 可审计 | 继续由模型 runner 提供 expected output，不把 scheduler evidence 当成模型正确性 | 否 |
...
这些状态是故意保守的。真实 bounded runtime 可以把 `--iterations`、`--warmUp`、`--duration`、有效的 `--streams/--infStreams`、`--idleTime`、`--avgRuns` 和 `--percentile` 标为 applied；被 `--infStreams` 覆盖的 `--streams` 仍是 parse-only。`--batch`、TRT10/11 的 `--minTiming`、`--sleepTime`、`--threads`、`--useSpinWait`、`--useCudaGraph`、`--noDataTransfers` 以及其余没有真实行为证据的参数不能写成已完成。`--avgTiming` 和 TRT8 legacy `--minTiming` 只代表 builder-config readback，timing-cache lifecycle 也只代表构建缓存证据。
