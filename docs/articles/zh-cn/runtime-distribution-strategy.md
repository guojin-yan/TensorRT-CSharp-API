...
Ubuntu 22.04 和 Ubuntu 24.04 hosted Linux 包已经有远程发布证据。Ubuntu 20.04 已建模为 hosted-container 发布线，需要单独补齐发布证据。未来 ARM/Jetson/非 Ubuntu 线仍需要匹配 package identity、runner 或 container、官方依赖源和 package-consumer 证据后才能发布。
...
- Ubuntu 22.04 和 Ubuntu 24.04 hosted Linux 包已有远程发布证据；Ubuntu 20.04 是下一条要补齐的 hosted-container 发布线，ARM/Jetson/非 Ubuntu 仍是未来独立包线。
