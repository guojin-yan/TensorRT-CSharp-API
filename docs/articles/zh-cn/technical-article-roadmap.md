...
| 103 | CUDA 安全边界 | Stream Capture To Graph 的 owner-safe session | `cudaStreamBeginCaptureToGraph`、CUDA 12.3+ guard、stream/graph dispose 阻止、same-graph handle 验证、deferred history、ToGraph smoke | `docs/articles/zh-cn/cuda-stream-capture-to-graph-owner-safety.md`、`artifacts/interface-coverage/cuda-stream-capture-to-graph-candidate-audit.md`、`src/JYPPX.CudaSharp/Streams/CudaStreamCaptureToGraphSession.cs`、`smoke/CudaGraphSmokeRunner` | 兼容 CUDA 主机；文章与 smoke 不是 clean package-consumer proof | 完整教程已收口 |
