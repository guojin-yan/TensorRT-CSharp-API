# 通过公开 NuGet 包使用 TensorRT DebugListener：从调试张量到真实回调

> 项目：TensorRtSharp4.0
>
> 主要库：`JYPPX.TensorRtSharp`、`JYPPX.CudaSharp`、`jyppxtrtbridge`
>
> 示例：`tests/fixtures/package-consumers/DebugListener.PackageConsumer`
>
...
在仓库外创建项目，并从公开 NuGet 源引用 managed 包和与主机矩阵匹配的 bridge-only 包：
