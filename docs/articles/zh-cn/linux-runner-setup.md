...
当仓库 secret `RUNNER_AUDIT_TOKEN` 可用时，`release-bundle.yml` 会在创建 GitHub Release 前检查 runner 可用性。Ubuntu 20.04 self-hosted 打包是严格预检：必须提供该审计 token，并要求仓库存在在线 runner，且包含 `self-hosted`、`linux`、`x64`、`ubuntu-20.04` 标签。
