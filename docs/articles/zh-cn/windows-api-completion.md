...
Runtime 与 Refitter 的 diagnostic summary 也已归入 `TensorRtRuntimeDiagnosticSummary.cs` 和
`TensorRtRefitterDiagnosticSummary.cs`。该移动保留 error-recorder、named-weight/dynamic-range inventory 与 diagnostic
count 的复制语义，不改变 refit/runtime 生命周期，也不把 summary 解释为运行证明。
