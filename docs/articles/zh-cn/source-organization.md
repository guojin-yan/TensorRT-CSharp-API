...
DebugListener native attach-entry runtime scaffold 与 attach-vtable safety gate 的 result 也已归入同名文件。原
362 行 runtime scaffold 分为 178 行 evaluator 与 189 行 result；原 357 行 safety gate 分为 166 行 evaluator 与
196 行 result。布局门禁可重组拆分前 Git blob `d90e00c816e24dc0854388c5f4562b8d55c53ab8` 与
...
Callback Core 的 algorithm snapshot design gate 与 MemoryAllocation 的 allocator interface-info design gate result
现在也各自归入同名文件。原 314 行 algorithm gate 分为 113 行 evaluator 与 207 行 result；原 283 行 allocator
interface-info gate 分为 114 行 evaluator 与 175 行 result。布局门禁可重组拆分前 Git blob
...
`TensorRtExecutionContextRuntimeDiagnosticSummary.cs` 与 `TensorRtEngineDeploymentSummary.cs` 现在分别拥有
剩余的 execution runtime-diagnostic summary 与 engine deployment summary。原 411 行 runtime-diagnostic snapshot
分为 198 行 snapshot 与 217 行 summary；原 380 行 engine deployment snapshot 分为 251 行 snapshot 与 133 行
summary。布局门禁可重组 Git blob `46eea3f13e19f01f9b927738c2c9dbad76172feb` 与
