...
- `package-managed.yml` 只固定打包并验证核心 managed 包 `JYPPX.TensorRT.CSharp.API`。`YoloVision` 和 `Classification` 只属于 samples 项目，严禁上传。发布必须同时满足 `owner_publish_approved=true`、正式仓库 owner、精确 ID/版本/source-commit allowlist；`publish_to_nuget=true` 时还要求仓库 secret `NUGET_API_KEY` 是纯文本 ASCII 的 nuget.org API key，并对核心 managed package 或其所属账号/组织拥有 push 权限。nuget.org `403` 是不可重试的权限错误，必须先替换失效、过期或 scope 不足的 key。
