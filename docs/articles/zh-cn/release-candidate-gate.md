...
- `package-managed.yml` 在 `publish_to_nuget=true` 时，如果仓库 secret `NUGET_API_KEY` 存在，就会使用该值，并要求它是纯文本 ASCII 的 nuget.org API key。也可以不设置该 secret，让 self-hosted Windows runner 使用本机 NuGet 配置兜底；本机 API key 必须配置在 `https://www.nuget.org` 上。不要填加密后的本机凭据或其它机器导出的 token 片段。
