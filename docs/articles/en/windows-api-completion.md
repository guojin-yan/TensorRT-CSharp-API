...
The validated Windows TensorRT 8.6 package previously raised a vendor SEH exception inside `nvonnxparser::createParser`. The bridge now wraps only that vendor call in the same native SEH and C++ exception-to-status boundary used by runtime and builder creation. Failure clears the parser output and cannot create an owner handle; success continues through the existing owner-backed handle. A dedicated TensorRtExec child process on TensorRT 8.6/CUDA 12.1/cuDNN 8.9 has completed MNIST ONNX parse, engine build and round trip, enqueue, and a 10-value zero-mismatch reference comparison. This remains local synthetic-input runtime evidence, not model-accuracy, package-consumer, or release proof.
...
The twenty-sixth Windows API batch raised the manifest count to `801` by adding builder-config profile stream/profile-count/calibration-profile APIs, detailed ONNX parser diagnostic APIs, CUDA stream capture status/info, and synchronous/asynchronous pitched 3D fill helpers. It originally made the TensorRT 8 Windows ONNX parser boundary explicit by returning `NotSupported` after reproducing a vendor SEH in `nvonnxparser::createParser`; the current bridge has advanced that historical guard to the native exception-to-status boundary described above.
