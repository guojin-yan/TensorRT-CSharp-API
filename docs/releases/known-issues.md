...
| KI-003 | Controlled | 用户安装命令和仓库案例共享规则均固定为正式版 `4.0.0`。由于 NuGet 源中存在 API 不兼容的历史 4.x 包，不能使用会选中旧包的宽泛 `4.*` 或 `--prerelease`。 | 维护的稳定版本前进时只更新 `build/JYPPX.PublicSamplePackages.props`，同步公开安装文案，并运行全部案例 restore graph 检查。 |
| KI-004 | Closed (15/15) | 部分历史证据文件和旧文章文件名仍含 `local-package-consumer`，它们记录的是发布前本地 feed 证明，不代表当前公共包消费。 | 全部 15 篇教程已改为公开包优先或明确跳转到当前公开教程，并在正文中隔离 local-feed 历史证据；历史文件名继续保留，且不进入公开导航。 |
