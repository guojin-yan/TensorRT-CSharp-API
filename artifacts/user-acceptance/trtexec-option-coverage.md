...
| Engine packaging / refit | `--versionCompatible`、`--excludeLeanRuntime`、`--stripWeights`、`--refit`、`--refitFromOnnx`、`--saveRefittedEngine`、`--allowWeightStreaming`、`--weightStreamingBudget` | OnnxToEngine / TensorRtExec CLI / TensorRtExec WinForms | TRT10 完成 copied inventory、parser load、engine commit、serialization `ExcludeWeights` clear/readback、distinct full-weight persisted plan、原 owner 释放、独立 reload、enqueue 和 fixed full-weight baseline exact match；TRT8 guard、TRT11 dependency probe 保持真实分类 |
