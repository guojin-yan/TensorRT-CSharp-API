...
| Engine packaging / refit | `--versionCompatible`、`--excludeLeanRuntime`、`--stripWeights`、`--refit`、`--allowWeightStreaming`、`--weightStreamingBudget` | OnnxToEngine / TensorRtExec CLI / TensorRtExec WinForms | 支持按版本 set/readback；TRT10/11 budget 在 context 前应用；TRT8 不支持项保持 parse-only，且不把本地策略证据冒充跨版本部署或发布证明 |
