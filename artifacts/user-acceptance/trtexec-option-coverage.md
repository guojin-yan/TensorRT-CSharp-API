...
报告现在输出 `OptionImplementationStatus`，将参数分为 `ParsedOptions`、`AppliedOptions` 和 `ParseOnlyOptions`。该字段用于防止把 parser/report 覆盖误读成真实 TensorRT 行为覆盖：I/O 与 layer precision policy 只有在真实 set/readback match 时才进入 applied；TRT11 已移除的 setters、advanced timing、engine packaging/refit、weight-streaming、safety/consistency 与 builder cache 仍必须留在 parse-only 边界内。
...
| IO / precision policies | `--inputIOFormats`、`--outputIOFormats`、`--precisionConstraints`、`--layerPrecisions`、`--layerOutputTypes` | OnnxToEngine / TensorRtExec CLI / TensorRtExec WinForms | TRT8/10 typed set/readback；单 IO spec 可 broadcast，layer exact 优先 wildcard、后规则覆盖前规则；TRT11 只应用 type 已匹配的 IO format，移除的 setters 保持 parse-only |
| Engine packaging / refit | `--versionCompatible`、`--excludeLeanRuntime`、`--stripWeights`、`--refit`、`--refitFromOnnx`、`--allowWeightStreaming`、`--weightStreamingBudget` | OnnxToEngine / TensorRtExec CLI / TensorRtExec WinForms | TRT10 `--refitFromOnnx` 完成 copied inventory、parser load、engine commit、context gate、enqueue 和 full-weight baseline exact match；TRT8 guard、TRT11 dependency probe 和非持久化边界保持真实分类 |
...
- builder cache lifecycle 尚未开放；timing cache import/export 已有 typed owner，但仍只是 build-cache evidence。
