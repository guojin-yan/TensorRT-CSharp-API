...
The `4.0.0-preview.1` candidate has published all twelve Linux `.Bridge.nupkg` files to the GitHub Packages NuGet feed and the matching GitHub Release. The workflow now supports publishing those project-owned `.Bridge` packages to nuget.org, but this document does not claim that publication is complete until the official push and visibility checks pass. The six Windows packages remain blocked until a compatible Windows/x64 self-hosted runner with the required NVIDIA SDKs is online; the workflow must not substitute an incompatible hosted image. No CUDA, cuDNN, TensorRT, NVRTC, parser, plugin, or builder-resource library is included.
