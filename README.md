...
TensorRT 10/11 `IStreamReaderV2` now has an owner-safe public path and repository-external two-package proof. `TensorRtStreamReader` copies serialized plan bytes into an immutable native owner, keeps the owner alive through each returned engine, exposes only copied diagnostics, and rejects concurrent or post-disposal reuse. `eng/Test-StreamReaderLocalPackageConsumer.ps1` executes two real TensorRT 10.11 deserializations plus inference, verifies read/seek activity, then requires deferred disposal and truncated-plan failure. See `samples/StreamReader.PackageConsumer`, `samples/assets/stream-reader-local-package-consumer-tensorrt10.11-evidence.json`, and `docs/articles/zh-cn/stream-reader-local-package-consumer-tutorial.md`. TRT11 source and manifests are ready but not claimed as locally runtime-verified; legacy `IStreamReader` and `IStreamWriter` remain deferred. Nothing is published.
