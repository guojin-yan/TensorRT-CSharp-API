...
`--refitFromOnnx <path>` is a managed extension for a complete in-memory stripped-plan lifecycle. It requires an explicit `--onnx` build plus `--stripWeights --refit`; load-engine mode is rejected so the ONNX build source and refit source cannot be confused. On TRT10/11 the tool deserializes the stripped plan, copies missing/all refit inventory, runs parser-refitter `RefitFromFile`, commits with `RefitCudaEngine`, captures copied parser diagnostics, and requires zero missing weights/errors before any execution context is created. The saved engine path remains the stripped artifact. This proves neither refitted-plan persistence nor package-consumer runtime.
