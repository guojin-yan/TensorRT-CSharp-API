...
`--builderOptimizationLevel` and `--maxAuxStreams` are applied to the TensorRT builder config when the runtime reaches the build stage. `--memPoolSize` known pool tokens (`workspace`, `dlaSRAM`, `dlaLocalDRAM`, `dlaGlobalDRAM`, `tacticDRAM`, `tacticSharedMem`) are applied through `SetMemoryPoolLimit` and verified with `GetMemoryPoolLimit`. `--device` runs the operation on a dedicated host thread and reads back the current CUDA device. Real builds set/read back `--useDLACore`, `--allowGPUFallback`, `--tacticSources`, `--directIO`, and `--sparsity=enable|disable`; `--stronglyTyped` uses the TRT10 raw bit while TRT11 relies on its always-strongly-typed contract. `--inputIOFormats` and `--outputIOFormats` validate official type/format grammar and apply tensor type/format constraints; `--precisionConstraints`, `--layerPrecisions`, and `--layerOutputTypes` apply with exact-name precedence, one-wildcard matching, later-rule override, and readback on TRT8/10. TRT11 applies IO formats only when the inferred type already matches and keeps removed layer setters behind explicit guards. TRT8 strongly typed, `--sparsity=force`, and `--calib` remain explicit parse-only controls. `--shapes` and `--inputShapes` seed min/opt/max profiles when no explicit triplet is supplied.

`--noDataTransfers`, `--useSpinWait`, boolean `--threads`, and `--useCudaGraph` are applied by compatible bounded runtime; `--sleepTime` remains parse-only because CPU sleeping is not the official device-side launch gap. `--safe`, `--consistency`, `--builderCache`, and `--noBuilderCache` remain parse/report-only; cache switches are mutually exclusive. Applied scheduler or deployment controls never claim DLA model execution, calibrator callbacks, plugin loading, model-specific correctness, safety runtime execution, or package-consumer-runtime proof.
