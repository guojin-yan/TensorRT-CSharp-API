...
- Engine packaging and device policy: packaging/refit/debug controls remain diagnostic. `--device` selects CUDA on a dedicated host thread; `--useDLACore` validates the builder-reported core count and applies DLA config; `--allowGPUFallback` is set/read back. These controls do not by themselves prove DLA model execution.
