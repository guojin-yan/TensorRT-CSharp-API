...
The official THU-MIG YOLOv10n v1.1 ONNX path is now backed by source-tree `real-model-runtime` evidence for `[1,300,6]` output. Run `eng/Acquire-YoloV10OfficialAssets.ps1` to acquire hash-pinned AGPL-3.0 assets in a repository-external workspace. The closure record is `artifacts/interface-coverage/yolov10-official-runtime-proof-closure.json`; it proves a local source-tree TensorRT enqueue and managed end-to-end decode only, not package-consumer-runtime, public redistribution approval, or publish readiness.
