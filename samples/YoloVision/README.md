...
The official THU-MIG YOLOv10n v1.1 ONNX path is backed by source-tree `real-model-runtime` evidence and isolated local three-package `local-package-consumer-runtime` evidence for `[1,300,6]` output. Run `eng/Acquire-YoloV10OfficialAssets.ps1` to acquire hash-pinned AGPL-3.0 assets in a repository-external workspace. Use `eng/Test-YoloVisionLocalPackageConsumer.ps1 -Scenario yolov10-detection` for the package path; the committed record is `samples/assets/yolovision-yolov10n-local-package-consumer-runtime-evidence.json`. Neither record proves public-feed availability, public redistribution approval, or publish readiness, and the package record explicitly makes no independent raw-tensor comparison claim.
...
```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\eng\Test-YoloVisionSegmentationLocalPackageConsumer.ps1 -PackageVersion 4.0.0
```

It repeats both raw output references, writes source-image mask artifacts, compares four masks with an independent Ultralytics/PyTorch CPU run, and requires a one-value reference mutation plus a one-byte mask mutation to fail closed. The compact record is `samples/assets/yolovision-yolov8n-seg-local-package-consumer-runtime-evidence.json`; the walkthrough is `docs/articles/zh-cn/yolovision-yolov8-seg-local-package-consumer-tutorial.md`. This remains local-file-feed evidence only.
