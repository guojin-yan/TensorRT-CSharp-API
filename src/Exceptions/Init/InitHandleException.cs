...
namespace JYPPX.TensorRtSharp.Exceptions
{
    ...
    /// </summary>
    static class InitHandleException
    {
        /// <summary>
        ...
        public static void handler(InitExceptionStatus status)
        {
            if (InitExceptionStatus.NotOccurred == status)
            {
                return;
            }
            else
            {
                throw new InitException(status, Marshal.PtrToStringAnsi(NativeMethods.GetLastErrMsg())); ;
            }
