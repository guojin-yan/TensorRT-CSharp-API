...
function New-BackfillLane {
  ...
  $ready = $present -and ($ReadyStates -contains $state)
...
$publicPackageDownloadProofValidation = Read-JsonOrNull "artifacts\final-release\public-package-download-proof-input-validation.json"
...
$lanes = @(
  New-BackfillLane -Id "github-source-head-status" -Artifact "artifacts/final-release/github-publish-and-ci-status-snapshot-validation.json" -Record $githubStatusValidation -StateProperty "validationState" -DefaultState "missing-github-publish-and-ci-status-snapshot-validation" -ReadyStates @("blocked-github-actions-and-public-publish-proof-required") -RequiredEvidence "Read-only remote HEAD containment snapshot plus explicit missing CI/publish proof classification."
  New-BackfillLane -Id "github-actions-run-proof" -Artifact "artifacts/final-release/github-actions-run-evidence-import.json" -Record (Read-JsonOrNull "artifacts\final-release\github-actions-run-evidence-import.json") -StateProperty "evidenceState" -DefaultState "missing-github-actions-run-evidence-import" -ReadyStates @("github-actions-run-evidence-ready") -RequiredEvidence "Real GitHub Actions workflow run URL, run id, head SHA, conclusion, log hash, and artifact hash."
  New-BackfillLane -Id "owner-public-publish-result" -Artifact "artifacts/final-release/owner-public-publish-execution-result-candidate-validation.json" -Record $ownerPublishResultCandidateValidation -StateProperty "validationState" -DefaultState "missing-owner-public-publish-execution-result-candidate-validation" -ReadyStates @("owner-public-publish-execution-result-candidate-ready") -RequiredEvidence "Owner-supplied public publish result with package identity, public URLs, hashes, transcript hashes, reviewer, and authorization linkage."
  New-BackfillLane -Id "public-package-download-proof" -Artifact "artifacts/final-release/public-package-download-proof-input-validation.json" -Record $publicPackageDownloadProofValidation -StateProperty "validationState" -DefaultState "missing-public-package-download-proof-input-validation" -ReadyStates @("public-package-download-proof-input-ready") -RequiredEvidence "Downloaded public package URL/source, SHA256, package identity, and non-local source proof."
  New-BackfillLane -Id "post-publish-clean-consumer-proof" -Artifact "artifacts/final-release/post-publish-clean-consumer-proof-result-validation.json" -Record $postPublishCleanConsumerProofResultValidation -StateProperty "validationState" -DefaultState "missing-post-publish-clean-consumer-proof-result-validation" -ReadyStates @("post-publish-clean-consumer-proof-result-validation-ready") -RequiredEvidence "Repository-external clean consumer restore/build/run logs, host metadata, package source, exit code, and SHA256 values after public publication."
