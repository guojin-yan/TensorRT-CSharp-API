...
$articles = @(
  ...
  New-Article -Id 27 -Track "Plugin" -Title "Plugin Registry Inventory：安全只读 API 如何避免 borrowed pointer 泄露" -TargetAudience "TensorRT plugin 用户、维护者" -ArticleType "深入技术" -DependentSampleCodePaths @("src/JYPPX.TensorRtSharp/TensorRtPluginRegistryInventory.cs","native/src/tensorrt") -RequiresRealModelAssets $false -ModelAcquisitionPlaceholder "不需要模型资产" -ScreenshotsOrImagesNeeded @("plugin creator 表","只读 copy 模式图") -Readiness "outline-ready" -ProofBoundary "$commonBoundary plugin inventory 是只读诊断，不证明 plugin load/register/deregister 或 enqueue。"
  New-Article -Id 28 -Track "Diagnostics" -Title "Engine Inspector 与只读 Diagnostics：如何读懂 layer info 而不越界声明 proof" -TargetAudience "推理调优工程师" -ArticleType "深入技术" -DependentSampleCodePaths @("src/JYPPX.TensorRtSharp/TensorRtEngineInspector.Trt11Diagnostics.cs","applications/TensorRtExec") -RequiresRealModelAssets $true -ModelAcquisitionPlaceholder "owner 提供可公开 engine 或 ONNX、layer info 输出、license、SHA256" -ScreenshotsOrImagesNeeded @("layer info 表","profile 摘要") -Readiness "needs-real-model-assets" -ProofBoundary "$commonBoundary layer/profile diagnostics 不是输出正确性 proof。"
