...
$dualPackageRoutes = if ($null -eq $dualPackageMatrix) {
  @(
    ...
    [pscustomobject]@{ id = "github-packages-bridge"; distributionChannel = "GitHub Packages"; packageId = "JYPPX.TensorRT.CSharp.API.Runtime.<runtimePackageKey>.Bridge"; nextOwnerAction = "owner-authorize-github-bridge-publish-and-import-clean-runtime-proof"; externalProofMissingReason = "github-bridge-restore-external-dependency-resolution-clean-smoke-missing"; postPublishProofMissingReason = "post-publish-github-bridge-clean-consumer-proof-missing"; canPublishPublicly = $false; canPublishGitHubPackages = $false; canClaimPackageConsumerRuntimeProof = $false; acceptsSubstituteProof = $false }
  )
}
else {
