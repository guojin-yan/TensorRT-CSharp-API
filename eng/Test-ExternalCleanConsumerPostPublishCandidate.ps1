...
$items += (New-OwnerValidationItem "external-path-hardened" ([bool](Get-PropertyOrDefault -Object $record -Name "rejectsLocalFeedProjectReferenceAndDirectNupkg" -DefaultValue $false) -and -not [bool](Get-PropertyOrDefault -Object $record -Name "externalWorkspacePathReady" -DefaultValue $true) -and @(Convert-ToArray (Get-PropertyOrDefault -Object $record -Name "externalWorkspacePathValidationReasons" -DefaultValue @())).Count -gt 0) "blocker" "Candidate must keep repository-external path checks visible and reject local feed/ProjectReference/direct nupkg substitutes.")
...
$validation = [pscustomobject]@{
  recordKind = "external-clean-consumer-post-publish-candidate-validation"; generatedAtUtc = [DateTimeOffset]::UtcNow.ToString("O"); validationState = $state; candidateState = [string]$record.candidateState; requiredFieldCount = [int]$record.requiredFieldCount; blockedFieldCount = [int]$record.blockedFieldCount; externalWorkspacePathReady = [bool](Get-PropertyOrDefault -Object $record -Name "externalWorkspacePathReady" -DefaultValue $false); externalWorkspacePathValidationReasonCount = @(Convert-ToArray (Get-PropertyOrDefault -Object $record -Name "externalWorkspacePathValidationReasons" -DefaultValue @())).Count; packageRestoreSourceUrlHost = [string](Get-PropertyOrDefault -Object $record -Name "packageRestoreSourceUrlHost" -DefaultValue ""); rejectsLocalFeedProjectReferenceAndDirectNupkg = [bool](Get-PropertyOrDefault -Object $record -Name "rejectsLocalFeedProjectReferenceAndDirectNupkg" -DefaultValue $false); failedBlockerCount = $failedBlockers.Count; validationItems = @($items); ownerActionRequired = $true; performsPublish = $false; usesPublishToken = $false; canPublishPublicly = $false; canCloseReleaseIssue = $false; canPromoteRuntimeProof = $false; isRuntimeExecutionProof = $false; isPostPublishProof = $false; isReleaseCloseProof = $false; boundary = "External clean consumer post-publish candidate validation is non-proof; not clean consumer proof, not runtime proof, not post-publish proof, not publish approval, not release close approval, and not package push."
}
