...
$items = @(
  ...
  New-ParityItem -OptionId "refitted-plan-persistence" -OfficialTrtexecOption "TensorRtExec extension: --saveRefittedEngine" -Tokens @("--saveRefittedEngine") -WinFormsTokens @("_saveRefittedEnginePath", "OnBrowseSaveRefittedEngine") -Status "implemented-trt10-persist-dispose-independent-reload" -ProofBoundary "Distinct persisted bytes, original owner disposal, independent reload, enqueue, baseline comparison, and separate local-feed PackageReference output-hash evidence are available; public-feed, post-publish and release proof remain external." -NextImplementationPath "src/JYPPX.TensorRtSharp.Tools; applications/TensorRtExec; tests/fixtures/package-consumers/RefittedPlan.PackageConsumer" -NextAction "Preserve overwrite and local-consumer isolation guards; collect public-feed external consumer and post-publish proof separately."
