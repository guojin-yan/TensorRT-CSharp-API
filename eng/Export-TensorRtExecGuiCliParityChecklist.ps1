...
$items = @(
  ...
  New-ParityItem -OptionId "timing-cache" -OfficialTrtexecOption "--timingCacheFile / --timingCache / --exportTimingCache" -Tokens @("--timingCacheFile", "--timingCache", "--exportTimingCache") -WinFormsTokens @("_timingCachePath", "_exportTimingCachePath") -Status "parse-report-only" -ProofBoundary "Timing cache paths do not prove cache import/export lifecycle." -NextImplementationPath "applications/TensorRtExec; native/src/tensorrt" -NextAction "Add lifecycle smoke only after ownership is safe."
  ...
  New-ParityItem -OptionId "runtime-benchmark" -OfficialTrtexecOption "--iterations / --warmUp / --duration / --streams / --avgRuns / --percentile" -Tokens @("--iterations", "--warmUp", "--duration", "--streams", "--avgRuns", "--percentile") -WinFormsTokens @("_iterations", "_warmUp", "_duration", "_streams", "_avgRuns", "_percentile") -Status "implemented-report" -ProofBoundary "Benchmark-shaped settings for external ONNX are not runtime proof without binding/output semantics." -NextImplementationPath "applications/TensorRtExec" -NextAction "Require real model logs and hashes before proof promotion."
  ...
  New-ParityItem -OptionId "device-dla" -OfficialTrtexecOption "--device / --useDLACore / --allowGPUFallback" -Tokens @("--device", "--useDLACore", "--allowGPUFallback") -WinFormsTokens @("_deviceOrdinal", "_dlaCore", "_allowGpuFallback") -Status "diagnostic" -ProofBoundary "Device/DLA intent requires host/model-specific evidence." -NextImplementationPath "applications/TensorRtExec" -NextAction "Record compatible host evidence in owner proof records."
)
