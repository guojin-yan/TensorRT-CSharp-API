...
$items = @(
  ...
  New-ParityItem -OptionId "onnx-stripped-plan-refit" -OfficialTrtexecOption "--refitFromOnnx" -Tokens @("--refitFromOnnx") -WinFormsTokens @("_refitFromOnnxPath", "OnBrowseRefitOnnx") -Status "implemented-trt10-trt11-refit-lifecycle-with-version-guards" -ProofBoundary "TRT10/11 copied inventory, parser load, engine commit, context gate, and enqueue are local source-tree evidence; the TRT11 reference is unreviewed and neither line proves model accuracy or package consumption." -NextImplementationPath "src/JYPPX.TensorRtSharp.Tools; applications/TensorRtExec" -NextAction "Keep TRT8 guarded; collect TRT11 version-compatible, weight-streaming, and external lean-runtime evidence separately."
