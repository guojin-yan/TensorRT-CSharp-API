...
$items = @(
  ...
  New-ParityItem -OptionId "io-formats" -OfficialTrtexecOption "--inputIOFormats / --outputIOFormats" -Tokens @("--inputIOFormats", "--outputIOFormats") -WinFormsTokens @("_inputIoFormats", "_outputIoFormats") -Status "implemented-build-readback-with-version-guards" -ProofBoundary "Tensor type/format application and readback are host build evidence; they do not prove model output correctness or package-consumer runtime behavior." -NextImplementationPath "applications/TensorRtExec; src/JYPPX.TensorRtSharp.Tools" -NextAction "Keep TRT11 type-mismatch guards and collect real-model proof separately."
  New-ParityItem -OptionId "layer-precision-policy" -OfficialTrtexecOption "--precisionConstraints / --layerPrecisions / --layerOutputTypes" -Tokens @("--precisionConstraints", "--layerPrecisions", "--layerOutputTypes") -WinFormsTokens @("_precisionConstraints", "_layerPrecisions", "_layerOutputTypes") -Status "implemented-build-readback-with-version-guards" -ProofBoundary "TRT8/10 layer policy application and readback are host build evidence; TRT11 removed setters remain parse-only, and neither path is generic runtime proof." -NextImplementationPath "applications/TensorRtExec; src/JYPPX.TensorRtSharp.Tools" -NextAction "Preserve TRT11 parse-only classification and attach model-specific runtime proof only to owner records."
  ...
  New-ParityItem -OptionId "plugins" -OfficialTrtexecOption "--plugins" -Tokens @("--plugins") -WinFormsTokens @("_plugins") -Status "diagnostic" -ProofBoundary "Plugin paths/inventory diagnostics do not prove plugin library load/register/deregister." -NextImplementationPath "src/JYPPX.TensorRtSharp/Plugins/TensorRtPluginRegistryInventory.cs" -NextAction "Keep copied inventory metadata read-only."
  ...
  New-ParityItem -OptionId "wait-idle-controls" -OfficialTrtexecOption "--sleepTime / --idleTime" -Tokens @("--sleepTime", "--idleTime") -WinFormsTokens @("_sleepTime", "_idleTime") -Status "implemented-bounded-runtime" -ProofBoundary "Sleep time queues one bridge-owned native host-function delay and fans one CUDA event out to all inference streams; idle time sleeps between measured rounds. These controls are scheduler behavior, not model correctness or package-consumer proof." -NextImplementationPath "native/src/cuda/api.cpp; src/JYPPX.CudaSharp; src/JYPPX.TensorRtSharp.Tools" -NextAction "Retain callback ownership guards and carry requested/applied fields into real-model and clean-consumer evidence."
  New-ParityItem -OptionId "binding-output" -OfficialTrtexecOption "--loadInputs / --dumpOutput / --dumpRawBindingsToFile / --exportOutput / --exportTimes" -Tokens @("--loadInputs", "--dumpOutput", "--dumpRawBindingsToFile", "--exportOutput", "--exportTimes") -WinFormsTokens @("_loadInputs", "_dumpOutput", "_dumpRawBindingsPath", "_exportOutputPath", "_exportTimesPath") -Status "bounded" -ProofBoundary "Build-only artifacts write boundary JSON; external ONNX binding output requires real model evidence." -NextImplementationPath "applications/YoloVision; applications/TensorRtExec" -NextAction "Use YoloVision owner templates to capture tensor role metadata."
  ...
  New-ParityItem -OptionId "onnx-stripped-plan-refit" -OfficialTrtexecOption "--refitFromOnnx" -Tokens @("--refitFromOnnx") -WinFormsTokens @("_refitFromOnnxPath", "OnBrowseRefitOnnx") -Status "implemented-trt10-trt11-refit-lifecycle-with-version-guards" -ProofBoundary "TRT10/11 copied inventory, parser load, engine commit, context gate, and enqueue are local source-tree evidence; TRT11 version-compatible+refit also has host-code readback and two-process reload evidence, but the reference is unreviewed and neither line proves model accuracy or package consumption." -NextImplementationPath "src/JYPPX.TensorRtSharp.Tools; applications/TensorRtExec" -NextAction "Keep TRT8 guarded; collect TRT11 weight-streaming and external lean-runtime evidence separately."
  New-ParityItem -OptionId "refitted-plan-persistence" -OfficialTrtexecOption "TensorRtExec extension: --saveRefittedEngine" -Tokens @("--saveRefittedEngine") -WinFormsTokens @("_saveRefittedEnginePath", "OnBrowseSaveRefittedEngine") -Status "implemented-trt10-trt11-persist-dispose-independent-reload" -ProofBoundary "TRT10/11 have distinct persisted bytes, original owner disposal, reload, and enqueue evidence; TRT11 also passes a loadEngine-only second process with identical output SHA. Only TRT10 has separate local-feed PackageReference evidence, and public-feed, post-publish and release proof remain external." -NextImplementationPath "src/JYPPX.TensorRtSharp.Tools; applications/TensorRtExec; tests/fixtures/package-consumers/RefittedPlan.PackageConsumer" -NextAction "Preserve overwrite and owner-isolation guards; collect public-feed external consumer and post-publish proof separately."
