...
$sourceStates = [pscustomobject]@{
  ...
  publicPackageDownloadProofOwnerExecutionPack = [string](Get-PropertyOrDefault -Object $publicPackageDownloadProofOwnerExecutionPackValidation -Name "validationState" -DefaultValue "missing-public-package-download-proof-owner-execution-pack-validation")
...
$gapFields = @(
  ...
  New-GapField -Id "dual-package-nuget-route-owner-proof" -Group "dual-package" -FieldPath "dualPackageRoutes.nuget-small-bridge-core.ownerAuthorization/externalProof/postPublishProof" -RequiredEvidence "Owner-authorized NuGet managed plus bridge-only public publish result, public package download proof, clean external consumer proof, and post-publish clean consumer proof." -Status "blocked by dual-package final close lane" -StrictValidators @("Test-DualPackagePublishPreflightMatrix.ps1 -Strict", "Test-FinalCloseGateConvergence.ps1 -Strict")
  New-GapField -Id "dual-package-github-bridge-route-owner-proof" -Group "dual-package" -FieldPath "dualPackageRoutes.github-packages-bridge.ownerAuthorization/externalProof/postPublishProof" -RequiredEvidence "Owner-authorized GitHub Packages bridge publish/restore proof, external NVIDIA dependency resolution report, clean external runtime smoke, and post-publish proof." -Status "blocked by dual-package final close lane" -StrictValidators @("Test-DualPackagePublishPreflightMatrix.ps1 -Strict", "Test-FinalCloseGateConvergence.ps1 -Strict")
...
$finalPublicProofPath = @(
  ...
  New-PublicProofStep -Order 4 -Id "public-package-download-owner-execution-pack" -Title "Public package download proof owner execution pack" -SourceArtifact "artifacts/final-release/public-package-download-proof-owner-execution-pack-validation.json" -ValidationArtifact "artifacts/final-release/public-package-download-proof-owner-execution-pack-validation.json" -CurrentState $sourceStates.publicPackageDownloadProofOwnerExecutionPack -RequiredReadyState "public-package-download-proof-owner-execution-ready-for-owner-review" -OwnerAction "Owner follows the public download proof execution pack, records public package URLs and hashes, and keeps candidate/template output out of proof classification until strict validation accepts real evidence." -StrictValidator "eng\Test-PublicPackageDownloadProofOwnerExecutionPack.ps1 -Strict" -BlockedReason "Public package download owner execution pack is still manual owner guidance and cannot be proof without real public download evidence."
  New-PublicProofStep -Order 5 -Id "post-publish-clean-consumer-proof-result" -Title "Post-publish clean consumer proof result" -SourceArtifact "artifacts/final-release/post-publish-clean-consumer-proof-result-validation.json" -ValidationArtifact "artifacts/final-release/post-publish-clean-consumer-proof-result-validation.json" -CurrentState $sourceStates.postPublishCleanConsumerProofResult -RequiredReadyState "post-publish-clean-consumer-proof-result-validation-ready plus proofCandidateReady=true and sourceProofLinkageReady=true" -OwnerAction "Owner runs repository-external clean consumer restore/build/runtime smoke after public publication and links ready GitHub Actions, Owner publish result, and public download proof." -StrictValidator "eng\Test-PostPublishCleanConsumerProofResult.ps1 -Strict" -BlockedReason "Post-publish clean consumer proof lacks real logs, hashes, host metadata, or upstream source proof linkage."
  ...
  New-PublicProofStep -Order 9 -Id "dual-package-final-close-lanes" -Title "Dual-package final close route lanes" -SourceArtifact "artifacts/final-release/final-close-gate-convergence-validation.json" -ValidationArtifact "artifacts/final-release/final-close-gate-convergence-validation.json" -CurrentState $sourceStates.finalCloseGateConvergenceValidation -RequiredReadyState "dualPackageRouteCount=2, dualPackageBlockedLaneCount=0, acceptsSubstituteProof=false, and all route owner proofs accepted" -OwnerAction "Owner must satisfy both NuGet managed and GitHub Packages bridge-only lanes with real publish/download/consumer/post-publish proof before final close; neither lane may contain NVIDIA vendor runtime assets." -StrictValidator "eng\Test-FinalCloseGateConvergence.ps1 -Strict" -BlockedReason "Dual-package final close lanes remain blocked until both route-specific Owner proof sets are real and accepted."
)
