...
$steps = @(
  ...
  New-ExecutionStep -Order 3 -ActionId $packageAction.id -Phase "package-consumer-runtime" -OwnerObjective "Choose the public package route and run a clean external package consumer from the public package source." -ExpectedInputArtifact $packageAction.ownerInputArtifact -ExpectedOutputArtifact $packageAction.requiredRecord -ValidatorCommand $packageAction.validator -FirstOwnerCommand $packageAction.firstCommand -FailureRepairHints @("Choose either github-full-dependency-package or nuget-core-api-plus-bridge-package.", "Run restore/build/smoke outside the repository and capture log/hash/exit code.", "Reject project-reference, local package-feed, and direct nupkg substitutes.") -LinkedArtifacts @($packageAction.ownerInputArtifact, $packageAction.requiredRecord, $packageAction.planningArtifact, $packageAction.executionKit) -ForbiddenSubstitutes $commonForbidden
