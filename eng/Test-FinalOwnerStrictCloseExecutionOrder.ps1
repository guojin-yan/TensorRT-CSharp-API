...
$items.Add((New-ValidationItem -Id "source-counts" -Passed ($sourceRecords.Count -ge 9 -and [int](Get-PropertyOrDefault -Object $record -Name "sourceRecordCount" -DefaultValue 0) -ge 9) -Severity "blocker" -Detail "Execution order must summarize all core owner execution source records.")) | Out-Null
...
foreach ($marker in @("template", "draft", "candidate", "dashboard", "dry-run", "build-only", "local feed", "ProjectReference", "direct nupkg", "direct .nupkg", "runbook as proof", "manual handoff as proof", "bundle-ready as proof", "blocked-by-cuda-driver", "dotnet nuget push")) {
