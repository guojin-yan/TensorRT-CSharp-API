[CmdletBinding()]
param(
  [string]$InputPath = "artifacts\final-release\release-candidate-final-owner-checklist.json",
...
if (-not (Test-Path -LiteralPath $resolvedInputPath -PathType Leaf)) { throw "Release candidate final owner checklist not found: $resolvedInputPath" }
if (-not [System.IO.Path]::IsPathRooted($OutputRoot)) { $OutputRoot = Join-Path $RepositoryRoot $OutputRoot }
New-Item -ItemType Directory -Path $OutputRoot -Force | Out-Null

$record = Get-Content -LiteralPath $resolvedInputPath -Raw -Encoding utf8 | ConvertFrom-Json
$itemsRaw = @((Get-PropertyOrDefault -Object $record -Name "checklistItems" -DefaultValue @()))
$blockedItems = @($itemsRaw | Where-Object { -not [bool](Get-PropertyOrDefault -Object $_ -Name "ready" -DefaultValue $false) })
$boundary = [string](Get-PropertyOrDefault -Object $record -Name "boundary" -DefaultValue "")

$items = New-Object System.Collections.Generic.List[object]
$items.Add((New-ValidationItem -Id "record-kind" -Passed ([string](Get-PropertyOrDefault -Object $record -Name "recordKind" -DefaultValue "") -eq "release-candidate-final-owner-checklist") -Severity "blocker" -Detail "recordKind must be release-candidate-final-owner-checklist.")) | Out-Null
$items.Add((New-ValidationItem -Id "state-blocked" -Passed ([string](Get-PropertyOrDefault -Object $record -Name "checklistState" -DefaultValue "") -eq "blocked-release-candidate-final-owner-checklist-owner-proof-required") -Severity "blocker" -Detail "Checklist must remain blocked until real owner proof exists.")) | Out-Null
$items.Add((New-ValidationItem -Id "items-present" -Passed ($itemsRaw.Count -ge 10) -Severity "blocker" -Detail "Checklist must expose final owner release steps.")) | Out-Null
$items.Add((New-ValidationItem -Id "owner-action-required" -Passed ($blockedItems.Count -eq 0) -Severity "action-required" -Detail "Owner must complete all checklist items.")) | Out-Null
...
$validationState = if ($failedBlockers.Count -gt 0) { "invalid-release-candidate-final-owner-checklist" } else { "blocked-release-candidate-final-owner-checklist-owner-proof-required" }

$validation = [pscustomobject]@{
  recordKind = "release-candidate-final-owner-checklist-validation"
  generatedAtUtc = [DateTimeOffset]::UtcNow.ToString("O")
  inputPath = $resolvedInputPath
  validationState = $validationState
  checklistState = [string](Get-PropertyOrDefault -Object $record -Name "checklistState" -DefaultValue "")
  checklistItemCount = $itemsRaw.Count
  ...
  boundary = "Validation checks final owner checklist shape only. It is not runtime proof, not post-publish proof, not publish approval, not release close approval, and not package push."
}

$jsonPath = Join-Path $OutputRoot "release-candidate-final-owner-checklist-validation.json"
$markdownPath = Join-Path $OutputRoot "release-candidate-final-owner-checklist-validation.md"
$validation | ConvertTo-Json -Depth 12 | Set-Content -LiteralPath $jsonPath -Encoding utf8
$rows = $validation.validationItems | ForEach-Object { "| ``$($_.id)`` | ``$($_.passed)`` | ``$($_.severity)`` | $($_.detail.Replace("|", "\|")) |" }
$markdown = @"
# Release Candidate Final Owner Checklist Validation
...
Write-Host "Release candidate final owner checklist validation written to $jsonPath"
Write-Host "ValidationState=$($validation.validationState) Items=$($validation.checklistItemCount) Blocked=$($validation.blockedChecklistItemCount) FailedBlockers=$($validation.failedBlockerCount) FailedActionRequired=$($validation.failedActionRequiredCount)"

if ($Strict.IsPresent -and $failedBlockers.Count -gt 0) { throw "Release candidate final owner checklist validation failed with $($failedBlockers.Count) blocker(s)." }
