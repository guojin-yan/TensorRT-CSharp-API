param(
  [string]$ManifestPath = "samples\assets\yolovision-yolov8n-obb-official-assets.json",
  [string]$OutputRoot = "",
  [string]$ReportPath = "artifacts\yolovision\yolov8n-obb-official-runtime\acquisition-report.json",
  [string]$PythonPath = "",
  [switch]$Offline
)
...
if ($manifest.recordKind -ne "yolovision-yolov8n-obb-official-asset-acquisition-manifest") {
  throw "Unexpected YOLOv8 OBB acquisition manifest recordKind."
}

if ([string]::IsNullOrWhiteSpace($OutputRoot)) {
  $OutputRoot = Join-Path $outerRoot "downloads\yolov8n-obb-ultralytics-v8.3.0"
}

$resolvedOutputRoot = [IO.Path]::GetFullPath($OutputRoot)
if (-not (Test-DriveIsNotC -Path $resolvedOutputRoot)) {
  throw "YOLOv8 OBB assets must not be downloaded to the C drive: $resolvedOutputRoot"
}
...
foreach ($asset in @($manifest.assets)) {
  ...
  if (-not $ready) {
    if ($Offline) {
      throw "YOLOv8 OBB asset '$($asset.id)' is missing or invalid in offline mode: $destination"
    }
  ...
  if (-not $ready) {
    throw "YOLOv8 OBB asset '$($asset.id)' failed length/SHA256 verification."
  }
...
$sourceImage = Join-Path $sourceRoot "boats.jpg"
...
if ($LASTEXITCODE -ne 0) {
  throw "Python/Pillow failed to derive the OBB PPM input."
}
...
if (-not $derivedReady) {
  throw "Derived YOLOv8 OBB PPM input failed dimensions/length/SHA256 verification."
}

$derivedLabels = Join-Path $derivedRoot ([string]$manifest.derivedLabels.fileName)
$labelText = (@($manifest.modelContract.outputs[0].classNames) -join "`n") + "`n"
[IO.File]::WriteAllText($derivedLabels, $labelText, $utf8)
$labelsIdentity = Get-FileIdentity -Path $derivedLabels
$labelsReady = $labelsIdentity.length -eq [int64]$manifest.derivedLabels.expectedLength -and
  $labelsIdentity.sha256 -eq [string]$manifest.derivedLabels.expectedSha256
if (-not $labelsReady) {
  throw "Derived YOLOv8 OBB labels failed length/SHA256 verification."
}
...
$report = [pscustomobject][ordered]@{
  schemaVersion = 1
  recordKind = "yolovision-yolov8n-obb-official-asset-acquisition-report"
  ...
  upstreamImageRepository = [string]$manifest.upstreamImageRepository
  upstreamImageCommit = [string]$manifest.upstreamImageCommit
  ...
  derivedLabels = [pscustomobject][ordered]@{
    path = [IO.Path]::GetFullPath($derivedLabels)
    length = $labelsIdentity.length
    sha256 = $labelsIdentity.sha256
    verified = $labelsReady
    derivation = [string]$manifest.derivedLabels.derivation
  }
  ...
  boundary = "Acquisition verifies the exact official Release asset ID, source/image commit assets, derived PPM, and DOTA labels against repository-pinned lengths/SHA256 values. The upstream Release did not publish a digest. This report is not export, real-model-runtime, package-consumer, redistribution, post-publish, or release proof."
}
...
$markdown = @(
  "# YOLOv8n OBB Official Asset Acquisition",
  "",
  ...
  "- derived labels: ``$derivedLabels`` / ``$($labelsIdentity.sha256)``",
...
Write-Host "YOLOv8n OBB official assets acquired and verified."
...
Write-Host "DerivedLabelsSha256=$($labelsIdentity.sha256)"
