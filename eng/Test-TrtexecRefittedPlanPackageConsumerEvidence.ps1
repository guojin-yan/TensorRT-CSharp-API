...
function Test-FiniteNonNegativeFloat {
  ...
  $parsed = [single]0
  $parsedSuccessfully = [single]::TryParse(
    [string]$Value,
    [Globalization.NumberStyles]::Float,
    [Globalization.CultureInfo]::InvariantCulture,
    [ref]$parsed)
  return $parsedSuccessfully -and
    (-not [single]::IsNaN($parsed)) -and
    (-not [single]::IsInfinity($parsed)) -and
    ($parsed -ge [single]0)
}
...
Add-Check "relative-source-artifacts" ($artifacts.sourcePlan -notmatch '^[A-Za-z]:[\\/]' -and $artifacts.sourceInput -notmatch '^[A-Za-z]:[\\/]' -and $artifacts.sourceReference -notmatch '^[A-Za-z]:[\\/]') "$($artifacts.sourcePlan)/$($artifacts.sourceInput)/$($artifacts.sourceReference)"
...
Add-Check "reference-copy-distinct" ([bool]$artifacts.referenceCopyPathDistinct) $artifacts.referenceCopyPathDistinct
Add-Check "reference-copy-hash" ($artifacts.sourceReferenceSha256 -eq $artifacts.copiedReferenceSha256 -and (Test-Sha256 $artifacts.copiedReferenceSha256)) "$($artifacts.sourceReferenceSha256)/$($artifacts.copiedReferenceSha256)"
Add-Check "reference-contract" ($artifacts.referenceTensorName -eq "Plus214_Output_0" -and (@($artifacts.referenceShape) -join ",") -eq "1,10" -and [int]$artifacts.referenceElementCount -eq 10 -and $artifacts.referenceSourceClassification -eq "repository-mnist-runtime-output-derived-unreviewed") "$($artifacts.referenceTensorName)/$(@($artifacts.referenceShape) -join ",")/$($artifacts.referenceElementCount)/$($artifacts.referenceSourceClassification)"
Add-Check "reference-policy" (([single]($artifacts.referenceAbsoluteTolerance) -eq [single]0.0001) -and ([single]($artifacts.referenceRelativeTolerance) -eq [single]0.0001) -and $artifacts.referenceNaNPolicy -eq "reject" -and $artifacts.referenceInfinityPolicy -eq "exact") "$($artifacts.referenceAbsoluteTolerance)/$($artifacts.referenceRelativeTolerance)/$($artifacts.referenceNaNPolicy)/$($artifacts.referenceInfinityPolicy)"
...
Add-Check "command-shape" ($execution.commandShape -eq "dotnet run --project <consumer-project> -c Release --no-build -- <copied-plan> <copied-input> <raw-output> <expected-output-sha256> <copied-reference-json> <abs-tolerance> <rel-tolerance> <nan-policy> <infinity-policy>") $execution.commandShape
...
Add-Check "reference-validation-gates" ([bool]$runtime.referenceValidationCompleted -and [bool]$runtime.referenceValidationPassed -and [int]$runtime.referenceComparedElementCount -eq 10 -and [int]$runtime.referenceMismatchCount -eq 0 -and [int]$runtime.referenceFirstMismatchIndex -eq -1) "$($runtime.referenceValidationCompleted)/$($runtime.referenceValidationPassed)/$($runtime.referenceComparedElementCount)/$($runtime.referenceMismatchCount)/$($runtime.referenceFirstMismatchIndex)"
Add-Check "reference-validation-errors" ((Test-FiniteNonNegativeFloat $runtime.referenceMaximumAbsoluteError) -and (Test-FiniteNonNegativeFloat $runtime.referenceMaximumRelativeError)) "$($runtime.referenceMaximumAbsoluteError)/$($runtime.referenceMaximumRelativeError)"
...
Add-Check "unreviewed-reference-boundary" ($boundary.statement -match "unreviewed same-runtime MNIST regression baseline" -and $boundary.statement -match "not independent model-accuracy") $boundary.statement
