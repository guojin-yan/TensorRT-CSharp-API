...
$forbidden = @("template", "dashboard", "audit", "bundle", "local feed", "ProjectReference", "direct .nupkg", "queued workflow", "missing runner", "dry-run", "ready fixture")
$lanes = @(
  ...
  New-OwnerActionLane -Id "external-clean-consumer-logs" -Title "External clean consumer logs" -RequiredFields @("externalWorkspaceRoot", "restoreLogSha256", "buildLogSha256", "smokeLogSha256", "hostMetadataSha256", "noLocalSubstituteConfirmation") -Commands @("dotnet restore <external-consumer.csproj> --source <public-package-source>", "dotnet build <external-consumer.csproj> -c Release --no-restore", "dotnet run --project <external-consumer.csproj> -- --runtime-package-key <runtime-key>") -ForbiddenSubstitutes $forbidden
  New-OwnerActionLane -Id "yolovision-real-model-assets" -Title "YoloVision real model assets" -RequiredFields @("modelSourceUrl", "modelSha256", "labelsSha256", "inputImageSha256", "outputJsonSha256", "stdoutLogSha256", "stderrLogSha256", "hostMetadataSha256", "ownerReviewedAtUtc") -Commands @("dotnet run --project .\\samples\\YoloVision -- --model <model.onnx> --labels <labels.txt> --task <det|cls|seg|obb|pose|sem> --input-shape <shape>", "pwsh -NoProfile -File .\\eng\\Test-YoloVisionRealAssetOwnerProofInput.ps1 -Strict") -ForbiddenSubstitutes $forbidden
  New-OwnerActionLane -Id "article-publication-urls" -Title "Article publication URLs" -RequiredFields @("articleId", "articleTitle", "articlePublishUrl", "articlePublishedAtUtc", "articleScreenshotSha256", "linkedPublicPackageUrl", "linkedProofHash") -Commands @("Review docs/articles/zh-cn/publishing/article-roadmap-30plus.json", "Record public article URL only after Owner publish evidence exists") -ForbiddenSubstitutes $forbidden
  New-OwnerActionLane -Id "release-issue-close-material" -Title "Release Issue close material" -RequiredFields @("releaseIssueUrl", "ownerCloseDecision", "releaseEvidenceBundleSha256", "classificationAuditSha256", "postPublishProofSha256", "rollbackDecision", "knownLimitationsAcknowledgement") -Commands @("pwsh -NoProfile -File .\\eng\\Test-FinalRealProofImportAndCloseCandidatePack.ps1 -Strict", "pwsh -NoProfile -File .\\eng\\Test-ReleaseIssueCloseRecord.ps1 -Strict") -ForbiddenSubstitutes $forbidden
)
