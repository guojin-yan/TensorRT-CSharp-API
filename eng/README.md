...
| `Export-*` | 368 | 生成机器可读报告、候选包、Owner 输入模板和文章结果图；多数不执行发布 |
| `Acquire-*` | 10 | 固定来源和 SHA 的模型/资产获取入口 |
| `Sync-*` | 2 | 本地资产同步和校验入口 |
| `Invoke-*` | 12 | 组合编排或本机 smoke 入口 |
| `Import-*` | 25 | 导入 Owner 或外部运行证据 |
| `New-*` / `Collect-*` | 4 | 脚手架、源码归档和收集器 |
| 其他 | 31 | 公共函数、验证、签名、归档工具和人工入口等 |

803 个 PowerShell 脚本中，780 个能在仓库源码、workflow、测试或文档中找到脚本名引用。其余 23 个包含公共函数文件、人工入口和待审计候选，不能仅凭“没有字面引用”判定无用。目录另有 9 个 Python 辅助脚本，其中包括 1 个导出器、7 个独立 reference 工具和 1 个受控变异工具。
