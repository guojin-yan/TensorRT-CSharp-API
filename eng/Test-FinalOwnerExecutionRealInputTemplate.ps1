...
$items.Add((New-ValidationItem -Id "required-sections" -Passed (($template.PSObject.Properties.Name -contains "owner") -and ($template.PSObject.Properties.Name -contains "fieldValues") -and ($template.PSObject.Properties.Name -contains "fileEvidence") -and ($template.PSObject.Properties.Name -contains "hashEvidence") -and ($template.PSObject.Properties.Name -contains "hostMetadata") -and ($template.PSObject.Properties.Name -contains "packageMetadata") -and ($template.PSObject.Properties.Name -contains "postPublishEvidence") -and ($template.PSObject.Properties.Name -contains "rollbackReview") -and ($template.PSObject.Properties.Name -contains "finalCloseDecision") -and ($template.PSObject.Properties.Name -contains "strictValidatorOutputs") -and ($template.PSObject.Properties.Name -contains "nonSubstituteConfirmations")) -Severity "blocker" -Detail "Template must include all Owner real input sections.")) | Out-Null
$items.Add((New-ValidationItem -Id "field-count" -Passed ($fieldValueProperties.Count -ge 39) -Severity "blocker" -Detail "Template must map every skeleton field.")) | Out-Null
