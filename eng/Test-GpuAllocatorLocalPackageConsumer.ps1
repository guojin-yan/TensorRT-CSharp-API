...
if ([string]::IsNullOrWhiteSpace($OutputRoot)) {
  $OutputRoot = Join-Path $outerRoot "package-consumers\gpu-allocator-$RuntimePackageKey"
}
if ([string]::IsNullOrWhiteSpace($ReportDirectory)) {
  $ReportDirectory = Join-Path $RepositoryRoot "artifacts\callbacks\gpu-allocator-local-package-consumer\$RuntimePackageKey"
}
...
$projectPath = Join-Path $OutputRoot "GpuAllocatorPackageConsumer.csproj"
$programPath = Join-Path $OutputRoot "Program.cs"
$nugetConfigPath = Join-Path $OutputRoot "NuGet.config"
$stdoutPath = Join-Path $ReportDirectory "gpu-allocator-local-package-consumer.stdout.txt"
$reportPath = Join-Path $ReportDirectory "gpu-allocator-local-package-consumer-runtime.json"
...
"@
$project = Get-Content -LiteralPath (Join-Path $RepositoryRoot "samples\GpuAllocator.PackageConsumer\GpuAllocator.PackageConsumer.csproj.template") -Raw
$project = $project.
...
Copy-Item -LiteralPath (Join-Path $RepositoryRoot "samples\GpuAllocator.PackageConsumer\Program.cs") -Destination $programPath -Force
...
if ($run.exitCode -ne 0) {
  throw "External GPU allocator package consumer failed:`n$($run.text)"
}

$marker = $run.lines | Where-Object { $_.StartsWith("GpuAllocatorRealRuntime=Passed ", [StringComparison]::Ordinal) } | Select-Object -First 1
if ($null -eq $marker) {
  throw "GpuAllocatorRealRuntime=Passed marker was not emitted."
}
$finalMarker = $run.lines | Where-Object { $_.IndexOf("Passed=True Mode=GpuAllocatorRuntimeSmokeOnly", [StringComparison]::Ordinal) -ge 0 } | Select-Object -First 1
...
if ($builderInvocationCount -eq 0 -or ($builderAllocateCount + $builderReallocateCount) -eq 0 -or
  ...
  throw "External package-consumer marker did not satisfy callback, fail-closed, pointer-free, and zero-leak invariants: $marker"
}
...
$report = [pscustomobject][ordered]@{
  schemaVersion = 1
  recordKind = "gpu-allocator-local-package-consumer-runtime"
  ...
  rejectionNegative = [pscustomobject][ordered]@{
    passed = $true
    buildFailed = $rejectedBuildFailed
    rejectedCount = $rejectedCount
  }
  exceptionNegative = [pscustomobject][ordered]@{
    passed = $true
    buildFailed = $exceptionBuildFailed
    callbackFailureCount = $exceptionFailureCount
  }
...
Write-Host "GpuAllocatorLocalPackageConsumer=Passed"
Write-Host "  Report=$reportPath"
Write-Host "  Stdout=$stdoutPath"
Write-Host "  BuilderCallbacks=$builderInvocationCount LiveAllocations=$builderLiveAllocationCount"
Write-Host "  RejectedCount=$rejectedCount ExceptionCallbackFailures=$exceptionFailureCount"
