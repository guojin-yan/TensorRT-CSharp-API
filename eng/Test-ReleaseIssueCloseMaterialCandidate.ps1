...
$items += (New-OwnerValidationItem "manual-close-review-only" ([bool](Get-PropertyOrDefault -Object $record -Name "manualCloseReviewOnly" -DefaultValue $false) -and -not [bool](Get-PropertyOrDefault -Object $record -Name "ownerCloseDecisionReady" -DefaultValue $true) -and -not [bool](Get-PropertyOrDefault -Object $record -Name "manualCloseReviewConfirmationReady" -DefaultValue $true)) "blocker" "Close material candidate must keep final Owner close decision as manual review input only.")
...
$validation = [pscustomobject]@{
  recordKind = "release-issue-close-material-candidate-validation"; generatedAtUtc = [DateTimeOffset]::UtcNow.ToString("O"); validationState = $state; candidateState = [string]$record.candidateState; requiredFieldCount = [int]$record.requiredFieldCount; blockedFieldCount = [int]$record.blockedFieldCount; dependencyCandidateCount = [int](Get-PropertyOrDefault -Object $record -Name "dependencyCandidateCount" -DefaultValue 0); finalBridgePassed = [bool](Get-PropertyOrDefault -Object $record -Name "finalBridgePassed" -DefaultValue $false); ownerCloseDecisionReady = [bool](Get-PropertyOrDefault -Object $record -Name "ownerCloseDecisionReady" -DefaultValue $false); manualCloseReviewConfirmationReady = [bool](Get-PropertyOrDefault -Object $record -Name "manualCloseReviewConfirmationReady" -DefaultValue $false); manualCloseReviewOnly = [bool](Get-PropertyOrDefault -Object $record -Name "manualCloseReviewOnly" -DefaultValue $true); failedBlockerCount = $failedBlockers.Count; validationItems = @($items); ownerActionRequired = $true; performsPublish = $false; usesPublishToken = $false; canPublishPublicly = $false; canCloseReleaseIssue = $false; canPromoteRuntimeProof = $false; isRuntimeExecutionProof = $false; isPostPublishProof = $false; isReleaseCloseProof = $false; boundary = "Release Issue close material candidate validation is non-proof; not release close approval, not post-publish proof, not runtime proof, not publish approval, and not package push."
}
