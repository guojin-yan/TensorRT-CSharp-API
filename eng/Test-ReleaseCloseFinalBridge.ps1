...
$requiredRejected = @("template", "dashboard", "dry-run", "local-feed", "ProjectReference", "direct-nupkg", "queued-workflow", "staging-shape-valid-only", "public-package-hash-only", "validation-ready-without-owner-proof", "sample-build-only", "mock-output")
...
$items = @(
  ...
  New-OwnerValidationItem "gate-coverage" ([int](Get-PropertyOrDefault -Object $record -Name "gateCount" -DefaultValue 0) -ge 7 -and $gates.Count -ge 7 -and @($gates | Where-Object { [string]$_.id -eq "real-owner-proof-convergence" }).Count -eq 1) "blocker" "Bridge must inspect post-publish, rollback, final close, staging, hashes, and unified Owner proof convergence."
