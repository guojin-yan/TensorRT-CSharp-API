...
$items = @(
  New-CatalogItem -Name "MultiStream" -Kind "sample" -RelativePath "samples\Performance\01.MultiStream" -ProjectPath "samples\Performance\01.MultiStream\MultiStream.csproj" -Status "ready-to-run" -Command "dotnet run --project .\samples\Performance\01.MultiStream\MultiStream.csproj -c Debug" -Evidence "samples/Performance/01.MultiStream/README.md" -AssetRequirement "CUDA runtime and local bridge probing." -Purpose "CUDA stream/event and cross-stream wait tutorial." -Notes "User-facing low-asset sample; GPU/runtime compatibility is still required."
  New-CatalogItem -Name "DynamicShape" -Kind "sample" -RelativePath "samples\Inference\02.DynamicShapes" -ProjectPath "samples\Inference\02.DynamicShapes\DynamicShape.csproj" -Status "ready-to-run" -Command "dotnet run --project .\samples\Inference\02.DynamicShapes\DynamicShape.csproj -c Debug" -Evidence "samples/Inference/02.DynamicShapes/README.md" -AssetRequirement "TensorRT/CUDA runtime and local bridge probing." -Purpose "Dynamic shape, optimization profile, binding, and enqueue tutorial." -Notes "Useful first TensorRT sample after package consumer validation."
  New-CatalogItem -Name "InferenceBindings" -Kind "sample" -RelativePath "samples\Inference\01.Bindings" -ProjectPath "samples\Inference\01.Bindings\InferenceBindings.csproj" -Status "ready-to-run" -Command "dotnet run --project .\samples\Inference\01.Bindings\InferenceBindings.csproj -c Debug" -Evidence "samples/Inference/01.Bindings/README.md" -AssetRequirement "TensorRT/CUDA runtime and local bridge probing." -Purpose "High-level TensorRtInferenceBindings host/device workflow." -Notes "Shows user-facing binding shape without raw native pointers."
  New-CatalogItem -Name "OnnxToEngine" -Kind "sample" -RelativePath "applications\OnnxToEngine" -ProjectPath "applications\OnnxToEngine\OnnxToEngine.csproj" -Status "ready-to-run" -Command "dotnet run --project .\applications\OnnxToEngine\OnnxToEngine.csproj -c Debug" -Evidence "applications/OnnxToEngine/README.md" -AssetRequirement "TensorRT/CUDA runtime and ONNX parser availability for the selected TensorRT line." -Purpose "ONNX parse, engine build, serialize, and deserialize walkthrough." -Notes "Parser support remains version-guarded, especially on TensorRT 8 Windows."
  New-CatalogItem -Name "Classification" -Kind "sample" -RelativePath "samples\ComputerVision\01.Classification" -ProjectPath "samples\ComputerVision\01.Classification\Classification.csproj" -Status "asset-required" -Command "dotnet run --project .\samples\ComputerVision\01.Classification\Classification.csproj -c Debug -- --model .\models\classifier.onnx --labels .\models\labels.txt --input .\models\image.jpg" -Evidence "samples/ComputerVision/01.Classification/README.md; samples/assets/classification-assets.template.json; artifacts/user-acceptance/sample-asset-manifest-audit.json; artifacts/user-acceptance/sample-asset-acquisition-plan.json; artifacts/user-acceptance/sample-run-evidence-record-validation.json; artifacts/user-acceptance/real-model-owner-handoff.json" -AssetRequirement "User-provided classifier ONNX, labels, and input image with compatible redistribution rights. Manifest status: $classificationAssetStatus; proof classification: $classificationProofClassification; acquisition plan: $sampleAssetAcquisitionPlanState; runner evidence state: $sampleRunEvidenceValidationState; runner can promote real model runtime: $sampleRunEvidenceCanPromoteRealModelRuntime; runner proof classification: $sampleRunEvidenceProofClassification; owner handoff state: $realModelOwnerHandoffState; owner handoff can promote real model runtime: $realModelOwnerHandoffCanPromoteRealModelRuntime; sample run evidence record: $classificationSampleRunEvidenceRecord; sample run evidence validation: $classificationSampleRunEvidenceValidation; manifest runner cross-check: $classificationSampleRunEvidenceCrossCheckState." -Purpose "Image classification deployment tutorial." -Notes "Asset manifest audit status: $sampleAssetAuditStatus. Candidate manifests, build-only records, acquisition plans, owner handoffs, and owner-action-required runner evidence are not sample smoke passes."
  New-CatalogItem -Name "YoloVision" -Kind "sample" -RelativePath "applications\YoloVision" -ProjectPath "applications\YoloVision\YoloVision.csproj" -Status "asset-required" -Command "dotnet run --project .\applications\YoloVision\YoloVision.csproj -c Debug -- --model .\models\yolo.onnx --labels .\models\coco.names --input-data .\models\yolo-preprocessed-fp32.bin --input-shape 1x3x640x640 --nms-mode class-aware" -Evidence "applications/YoloVision/README.md; samples/assets/yolovision-assets.template.json; artifacts/user-acceptance/sample-asset-manifest-audit.json; artifacts/user-acceptance/sample-asset-acquisition-plan.json; artifacts/user-acceptance/sample-run-evidence-record-validation.json; artifacts/user-acceptance/real-model-owner-handoff.json" -AssetRequirement "User-provided YOLO-family ONNX, COCO labels, source image, and preprocessed input tensor with compatible redistribution rights. Manifest status: $yoloVisionAssetStatus; proof classification: $yoloVisionProofClassification; acquisition plan: $sampleAssetAcquisitionPlanState; runner evidence state: $sampleRunEvidenceValidationState; runner can promote real model runtime: $sampleRunEvidenceCanPromoteRealModelRuntime; runner proof classification: $sampleRunEvidenceProofClassification; owner handoff state: $realModelOwnerHandoffState; owner handoff can promote real model runtime: $realModelOwnerHandoffCanPromoteRealModelRuntime; sample run evidence record: $yoloVisionSampleRunEvidenceRecord; sample run evidence validation: $yoloVisionSampleRunEvidenceValidation; manifest runner cross-check: $yoloVisionSampleRunEvidenceCrossCheckState." -Purpose "YOLO-family vision deployment tutorial." -Notes "Asset manifest audit status: $sampleAssetAuditStatus. Candidate manifests, build-only records, acquisition plans, owner handoffs, and owner-action-required runner evidence are not sample smoke passes."
