...
$validation = [pscustomobject]@{
  recordKind = "article-publication-proof-candidate-validation"; generatedAtUtc = [DateTimeOffset]::UtcNow.ToString("O"); validationState = $state; candidateState = [string]$record.candidateState; requiredFieldCount = [int]$record.requiredFieldCount; blockedFieldCount = [int]$record.blockedFieldCount; failedBlockerCount = $failedBlockers.Count; validationItems = @($items); ownerActionRequired = $true; performsPublish = $false; usesPublishToken = $false; canPublishPublicly = $false; canCloseReleaseIssue = $false; canPromoteRuntimeProof = $false; isRuntimeExecutionProof = $false; isPostPublishProof = $false; isReleaseCloseProof = $false; boundary = "Article publication proof candidate validation is non-proof; not post-publish proof, not runtime proof, not publish approval, not release close approval, and not package push."
}
