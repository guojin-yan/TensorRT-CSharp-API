...
$requiredRouteIds = @("nuget-small-bridge-core", "github-packages-full-runtime")
...
$items.Add((New-ValidationItem -Id "route-count" -Passed ([int](Get-PropertyOrDefault -Object $record -Name "routeCount" -DefaultValue 0) -eq 2 -and $routes.Count -eq 2) -Severity "blocker" -Detail "Matrix must include exactly two routes: NuGet small bridge/core and GitHub Packages full runtime.")) | Out-Null
...
$items.Add((New-ValidationItem -Id "owner-actions" -Passed (($ownerActions -contains "owner-authorize-public-nuget-publish-and-import-clean-external-consumer-proof") -and ($ownerActions -contains "owner-authorize-github-packages-publish-and-import-credentialed-clean-runtime-proof")) -Severity "blocker" -Detail "Both route next owner actions must be present.")) | Out-Null
$items.Add((New-ValidationItem -Id "external-proof-gaps" -Passed (($externalProofMissingReasons -contains "public-package-download-and-clean-consumer-runtime-proof-missing") -and ($externalProofMissingReasons -contains "github-packages-restore-source-runtime-dll-resolution-clean-smoke-missing")) -Severity "blocker" -Detail "Both external proof missing reasons must be present.")) | Out-Null
$items.Add((New-ValidationItem -Id "post-publish-proof-gaps" -Passed (($postPublishProofMissingReasons -contains "post-publish-clean-consumer-proof-missing") -and ($postPublishProofMissingReasons -contains "post-publish-github-packages-clean-consumer-proof-missing")) -Severity "blocker" -Detail "Both post-publish proof missing reasons must be present.")) | Out-Null
