...
| Engine packaging/refit | `--versionCompatible --excludeLeanRuntime --stripWeights --refit --refitFromOnnx --allowWeightStreaming --weightStreamingBudget` | 无 | 可用 builder flag 真实 set/readback；TRT10/11 可执行 ONNX parser-refitter load + engine commit + context gate，预算也在 context 前设置；TRT8 保持 parse-only guard |
...
| Refit / weight streaming | `--refit`、`--refitFromOnnx`、`--allowWeightStreaming`、`--weightStreamingBudget` | version-guarded build/refit/runtime readback | `--refitFromOnnx` 要求 ONNX build + strip + refit，依次执行 copied inventory、parser load、engine commit 和 context gate；TRT10/11 budget 也在 context 前 set/get；TRT8 保持 parse-only |
...
`OptionImplementationStatus` 会把参数拆成 `ParsedOptions`、`AppliedOptions` 和 `ParseOnlyOptions`。这不是另一套完成度口径，而是防越级证据边界：除已有 builder、timing cache、deployment、I/O/layer 和 bounded-runtime 控制外，engine packaging/refit 只有真实 readback match 才 applied；`--refitFromOnnx` 还要求 parser load、engine commit、missing/error 清零和 context gate 全部成功，`--weightStreamingBudget` 只有 engine set/get match 才 applied。`--useCudaGraph` 只有所有 context capture/instantiate 成功后才 applied。`--batch`、TRT10/11 的 `--minTiming`、被 `--infStreams` 覆盖的 `--streams`、`--sleepTime`、TRT8 的 `--stronglyTyped`、`--sparsity=force`、TRT8 strip/streaming、TRT8 version-compatible+refit 冲突和其余无真实行为证明的选项继续保持 parse-only / probe-only。No-transfer run 不读取 output，也不声明 output match；builder/deployment evidence 不自动成为模型正确性或 package-consumer-runtime proof。
