...
| Engine packaging/refit | `--versionCompatible --excludeLeanRuntime --stripWeights --refit --allowWeightStreaming --weightStreamingBudget` | 无 | 可用 builder flag 真实 set/readback；TRT10/11 在 context 前设置预算，TRT8 的 strip/streaming 与 version-compatible+refit 冲突保持 parse-only |
...
| Refit / weight streaming | `--refit`、`--allowWeightStreaming`、`--weightStreamingBudget` | version-guarded build/runtime readback | refit flag 与可反序列化 engine 的 `IsRefittable` 读回；TRT10/11 支持 `-2`、`-1`、`0..100%` 和 bytes 预算并在 context 前 set/get；TRT8 streaming 以及 version-compatible+refit 冲突保持 parse-only |
