...
该 validator 检查 required 顶层字段、`NormalizedCommandSha256`、`OptionImplementationStatus`、`PreflightMetadata`、`LoadedEngineDiagnostics`、`CapabilityProbe` 和 `ReportBoundary.ForbiddenSubstitutes`。校验通过只能说明 report 可被机器审查，不能把 TensorRtExec report 或 OnnxToEngine report 提升成 `real-model-runtime` 或 `package-consumer-runtime` proof。
