...
| Runtime timing | `--iterations --warmUp --duration --streams --infStreams --idleTime --avgRuns --percentile` | 无 | compatible float engine 的 bounded runtime 会创建独立 execution context/stream，执行预热、次数+时长双下限、并发 stream、迭代间隔、平均窗口和百分位统计 |
| Advanced timing | `--avgTiming --minTiming --sleepTime --useSpinWait --threads --useCudaGraph` | 无 | `--avgTiming` 在真实 build 中调用 builder-config setter 并 read back；`--minTiming` 只在 TRT8 使用 legacy setter；其余 runtime mechanics 仍是 parse-only |
...
| Runtime benchmark | `--iterations`、`--warmUp`、`--duration`、`--streams`、`--avgRuns`、`--percentile` | bounded-runtime-scheduler | compatible float engine 执行真实 enqueue 与 GPU event 计时；外部模型没有 expected output 语义时不提升为真实 runtime proof |
...
`OptionImplementationStatus` 会把参数拆成 `ParsedOptions`、`AppliedOptions` 和 `ParseOnlyOptions`。这不是另一套完成度口径，而是防越级证据边界：`--builderOptimizationLevel`、workspace、shape profile、`--avgTiming`、TRT8 的 `--minTiming`、成功构建时的 timing-cache import/export，以及真实 bounded runtime 中的 `--iterations`、`--warmUp`、`--duration`、有效的 `--streams/--infStreams`、`--idleTime`、`--avgRuns` 和 `--percentile` 可标为 applied；`--batch`、TRT10/11 的 `--minTiming`、被 `--infStreams` 覆盖的 `--streams`、`--sleepTime`、`--threads`、`--useSpinWait`、`--useCudaGraph`、`--noDataTransfers` 和其余尚无 native/runtime 行为证明的选项继续保持 parse-only / probe-only。Scheduler evidence 只证明 bounded enqueue 调度，不自动成为模型正确性或 package-consumer-runtime proof。
