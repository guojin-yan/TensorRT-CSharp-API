...
| Output artifacts | `--loadInputs --dumpOutput --dumpRawBindingsToFile --exportOutput --exportTimes --exportProfile --saveProfile` | 无 | bounded runtime 会捕获全部 float output；控制台输出有界 preview，JSON 记录逐 tensor shape/长度/SHA256，raw 文件按 engine output 顺序拼接并生成 offset manifest |
