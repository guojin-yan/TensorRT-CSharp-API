...
| Diagnostic reports | `--dumpLayerInfo --exportLayerInfo --dumpProfile --separateProfileRun` | 无 | layer-info 在真实 build/load-engine 中复制 inspector 数据；detailed 模式导出合法 JSON 文档并进入顶层 `LayerInfoArtifact`，其他模式保留 one-line text；profile switches 仍是 intent/report evidence，所有诊断都不是 runtime proof |
...
TensorRtExec 的 build/load/runtime report 现在会把已有 `TensorRtEngineBindingReport` 复制到顶层 `BindingMetadata`，记录 input/output mode、dtype、engine/profile shape、location、format、vectorization、byte-size fallback 和 diagnostics；CLI 与 WinForms 通过同一个 `TensorRtExecReportFormatter` 展示相同的 tensor 摘要、最终状态和错误分类。YoloVision 在此基础上继续补充模型特定的 semantic role。所有这些数据都是 pointer-free deployment metadata，不是 output correctness、real-model-runtime 或 package-consumer-runtime proof。

Layer/profile diagnostic switch parity is now explicit: real build/load-engine paths use a copied `TensorRtEngineInspector` readback for `--dumpLayerInfo` and `--exportLayerInfo`. Detailed mode writes a self-describing JSON document and reports the top-level `LayerInfoArtifact` length/SHA256; other modes retain one-line text. Dry-run and unavailable dependencies remain parse/report-only. `--dumpProfile`、`--separateProfileRun`、`--exportProfile` 和 `--saveProfile` still record profiling intent. Layer artifacts and profile artifacts remain diagnostics, not runtime proof, unless a model-specific runner supplies real execution evidence.

2026-08-10 跨版本记录位于 `samples/assets/tensorrtexec-yolov8n-cls-cross-version-runtime-evidence.json`：TRT11.0/CUDA12.9 完成外部 YOLOv8n-cls build、engine round-trip、88 层 hashed JSON 和 1000 值零 mismatch 参考校验；TRT11 已移除的 precision setters 保持 parse-only。该文件中的 TRT8.6/CUDA12.1 `dependency-probe-only` 是原安全 guard 下的历史记录。当前 bridge 已在 `nvonnxparser::createParser` 调用点增加原生 SEH/C++ exception 到 status 的转换，并在独立 TensorRtExec 子进程中完成 MNIST parse、engine build/round-trip、enqueue 和 10 值零 mismatch comparison；compact evidence 位于 `artifacts/interface-coverage/trt8-windows-onnx-parser-seh-boundary-evidence.json`，严格检查为 `28/28`。该结果仍是 `synthetic-input-runtime`，不是 YOLOv8n-cls layer-info、模型准确率、package consumer 或发布证明。
...
`OptionImplementationStatus` 会把参数拆成 `ParsedOptions`、`AppliedOptions` 和 `ParseOnlyOptions`。`--refitFromOnnx` 要求 parser load、engine commit、missing/error 清零和 context gate 全部成功；`--saveRefittedEngine` 还要求 serialization config 已清除并回读 `ExcludeWeights`、持久化 artifact 与 stripped plan 不同、原 engine 在 reload 前释放、新 engine metadata gate 完整通过，才会进入 AppliedOptions。TensorRT 10/11 的完整权重 reload engine 可以不再保持 refittable，`ReloadEngineRefittable` 因而只作诊断，不是 context gate。TRT11 的 2026-08-10 证据位于 `artifacts/real-case/tensorrtexec-trt11-refit-lifecycle-20260810-091859/`：同进程 refit/reload 与 loadEngine-only 第二进程均通过 `69/69` strict report checks，输出 SHA 相同且 reference mismatch 为 0。该 reference 来自未审核的仓库 MNIST runtime 输出，所以仍是 `synthetic-input-runtime`，不是模型精度、包消费者或发布 proof。
